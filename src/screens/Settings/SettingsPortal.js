import React, { useState } from 'react'
import styled from 'styled-components'
import { AddTokenModal } from './AddTokenModal'
import useChromeStorageLocal from '../../hooks/useChromeStorageLocal'

const TextButton = styled.div`
  cursor: pointer;
`

export const SettingsPortal = () => {
  const [, token, setToken] = useChromeStorageLocal('token')
  const [updatingToken, setUpdatingToken] = useState(false)

  return (
    <div className="discussion-sidebar-item border-top-0 mt-0">
      <div className="details-reset details-overlay details-overlay-dark">
        <TextButton
          role="button"
          onClick={() => {
            setUpdatingToken(true)
          }}
        >
          <span className="text-bold link-gray-dark lock-toggle-link">
            <svg
              className="octicon octicon-trashcan"
              viewBox="0 0 12 19"
              version="1.1"
              width="12"
              height="16"
              aria-hidden="true"
            >
              <path
                fillRule="evenodd"
                d="M14 8.77v-1.6l-1.94-.64-.45-1.09.88-1.84-1.13-1.13-1.81.91-1.09-.45-.69-1.92h-1.6l-.63 1.94-1.11.45-1.84-.88-1.13 1.13.91 1.81-.45 1.09L0 7.23v1.59l1.94.64.45 1.09-.88 1.84 1.13 1.13 1.81-.91 1.09.45.69 1.92h1.59l.63-1.94 1.11-.45 1.84.88 1.13-1.13-.92-1.81.47-1.09L14 8.75v.02zM7 11c-1.66 0-3-1.34-3-3s1.34-3 3-3 3 1.34 3 3-1.34 3-3 3z"
              />
            </svg>{' '}
            <strong>Update OctoTask</strong>
          </span>
        </TextButton>
        {updatingToken && (
          <AddTokenModal
            initialValue={token}
            onSave={value => setToken(value)}
            onClose={() => setUpdatingToken(false)}
          />
        )}
      </div>
    </div>
  )
}
